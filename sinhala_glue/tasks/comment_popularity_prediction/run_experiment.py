...
from sinhala_glue.tasks.comment_popularity_prediction.evaluation import pearson_corr, spearman_corr, rmse
...
train['likes'] = scaler.fit_transform(train[['likes']])
test['likes'] = scaler.fit_transform(test[['likes']])

index = test['id'].to_list()
train = train.rename(columns={'comment': 'text_a', 'news_content': 'text_b', 'likes': 'labels'}).dropna()
...
for i in range(5):
    ...
    pearson = pearson_corr(test["likes"].tolist(), test["predictions"].tolist())
    spearman = spearman_corr(test["likes"].tolist(), test["predictions"].tolist())
    rmse_value = rmse(test["likes"].tolist(), test["predictions"].tolist())
