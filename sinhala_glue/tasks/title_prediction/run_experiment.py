...
from sinhala_glue.tasks.title_prediction.evaluation import macro_f1, weighted_f1
...
train = Dataset.to_pandas(
    load_dataset('sinhala-nlp/sinhala-headline-prediction', split='train', download_mode='force_redownload'))
...
for i in range(5):
    ...
    model_args.output_dir = os.path.join("outputs", "headline_prediction", processed_model_name)
    model_args.best_model_dir = os.path.join("outputs", "headline_prediction", processed_model_name, "best_model")
    model_args.cache_dir = os.path.join("cache_dir", "headline_prediction", processed_model_name)

    model_args.wandb_project = "Sinhala Headline Prediction"
