...
train = Dataset.to_pandas(load_dataset('sinhala-nlp/SOLD', split='train', download_mode='force_redownload'))
test = Dataset.to_pandas(load_dataset('sinhala-nlp/SOLD', split='test', download_mode='force_redownload'))

# index = test['id'].to_list()
train = train.rename(columns={'label': 'labels'}).dropna()
test = test.rename(columns={'label': 'labels'}).dropna()
...
for i in range(5):
    ...
    model_args.labels_list = ["OFF", "NOT"]

    processed_model_name = model_name.split("/")[1]

    model_args.output_dir = os.path.join("outputs", "offensive_language_detection", processed_model_name)
    model_args.best_model_dir = os.path.join("outputs", "offensive_language_detection", processed_model_name, "best_model")
    model_args.cache_dir = os.path.join("cache_dir", "offensive_language_detection", processed_model_name)

    model_args.wandb_project = "Sinhala Offensive Language Detection"
